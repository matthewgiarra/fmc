...
function rotationErrorHistogram_rotation_scaling(JOBLIST)
...
meanAbsErrorRotation = zeros(nRotBins, nScalingBins);
stdAbsErrorRotation = zeros(nRotBins, nScalingBins);

% Calculate the statistics for each bin
parfor x = 1 : nRotBins
    fprintf(1, [num2str(x) ' of ' num2str(nScalingBins) '\n']);
    for y = 1 : nScalingBins
        meanAbsErrorRotation(y, x) = mean(rotationAbsErrorMagnitude( scalingBins == y & rotBins == x ));
        stdAbsErrorRotation(y, x) = std(rotationAbsErrorMagnitude( scalingBins == y & rotBins == x ));
    end    
end

logAbsErrorRotation = log10(meanAbsErrorRotation);

% Contour levels
% contourLevels = linspace(log10(0.025), log10(3), 20);
contourLevels = log10(0.025) : 0.1 : log10(3);

% Absolute error of translation
f = figure(1);

% Make contour plot
contourf( rotBinEdges(1:end), scalingBinEdges(1:end), logAbsErrorRotation(1:end, 1:end), contourLevels); 
% contourf( rotBinEdges(1:end), scalingBinEdges(1:end), meanAbsErrorRotation(1:end, 1:end)); 

% Format the plot
axis square;
set(gca, 'ydir', 'normal');set(gcf, 'color', 'white')
% caxis([-2 1]);
caxis([log10(0.025), log10(3)]);
...
ticksLin = log10([0.025, 0.05, 0.1, 0.25, 0.5, 1, 3]);
...
title({'Average absolute errors of rotation estimate (degrees) ' ; ...
...
% Save the figure
print(1, '-depsc', ['~/Desktop/Histograms/eps/rotationError_rotation_scaling_hist_' caseName '_' num2str(regionHeight) 'x' num2str(regionWidth) '_' JOBFILE.CorrelationType '_linear.eps']);
saveas(f, ['~/Desktop/Histograms/fig/rotationError_rotation_scaling_hist_' caseName '_' num2str(regionHeight) 'x' num2str(regionWidth) '_' JOBFILE.CorrelationType '_linear.fig']);

% Save the histogram data
save(['~/Desktop/Histograms/data/rotationError_rotation_scaling_hist_' caseName...
    '_' num2str(regionHeight) 'x' num2str(regionWidth)...
    '_' JOBFILE.CorrelationType '_linear.mat'], 'JOBFILE', 'logAbsErrorRotation', 'scalingBinEdges', 'rotBinEdges', 'caseName', 'contourLevels', 'regionHeight', 'regionWidth', 'fSize');
