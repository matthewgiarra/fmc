...
function rotationErrorHistogram_rotation_scaling_linear(JOBLIST)
...
% Contour levels
contourLevels = linspace(-1.6, 0.5, 20);

% Absolute error of translation
f = figure(1);

% Make contour plot
contourf( rotBinEdges(1:end), scalingBinEdges(1:end), log10(meanAbsErrorRotation(1:end, 1:end)), contourLevels); 
...
ticksLin = log10([0.05, 0.1, 0.5, 1, 3]);
...
% Save the figure
print(1, '-depsc', ['~/Desktop/Histograms/eps/translationError_rotation_scaling_hist_' caseName '_' num2str(regionHeight) 'x' num2str(regionWidth) '_' JOBFILE.CorrelationType '_linear.eps']);
saveas(f, ['~/Desktop/Histograms/fig/translationError_rotation_scaling_hist_' caseName '_' num2str(regionHeight) 'x' num2str(regionWidth) '_' JOBFILE.CorrelationType '_linear.fig']);
...
save(['~/Desktop/Histograms/data/translationError_rotation_scaling_hist_' caseName...
    ...
    '_' JOBFILE.CorrelationType '_linear.mat'], 'JOBFILE', 'logAbsErrTranslation', 'scalingBinEdges', 'rotBinEdges', 'caseName', 'contourLevels', 'regionHeight', 'regionWidth', 'fSize');
