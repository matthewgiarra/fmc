function calculateExactSolutionsFullField_convergence(JOBLIST)
...
for n = 1 : nJobs
    ...
    % Number of passes
%     nPasses = length(firstVectorFile.X);
     nPasses = JobFile.JobOptions.NumberOfPasses;
        
    % Calculate the true solutions at the grid points for each pass.  
    for PASS = 1 : nPasses

        % Find the first iteration number that corresponds fo the pass
        % number PASS.
        p = find(firstVectorFile.PASSNUMBER == PASS, 1);
        
       
        % Read the grid points at which vectors were calculated
        X{PASS} = firstVectorFile.X{p};
        Y{PASS} = firstVectorFile.Y{p};
       ...
       % ParametersPath
        ...
        % gridpoints
        gridPointsX = (firstVectorFile.X{p}(:));
        gridPointsY = (firstVectorFile.Y{p}(:));
...
%         trueShearInside{PASS} = shearVect(abs(rTrue{PASS}) >= vorticityThreshold * max(rTrue{PASS}(:)));
    ...
    % Counter;
    c = 0;
    % Initialize number of deform passes vect
    nDeforms = zeros(nSets * nPairs, nPasses);
    iterationsPerPass = zeros(nSets * nPairs, nPasses);
    hasConverged = zeros(nSets * nPairs, nPasses);
    
    bigErr{nPasses} = [];
    bigErrInside{nPasses} = [];
...
%             disp(['Pair ' num2str(p) ' of ' num2str(nPairs)]);
            ...
            if exist(vectorFilePath, 'file')
                ...
                for PASS = 1 : nPasses 
                    ...
                    % Find the iteration number that corresponds
                    % to the last run of pass number specPass
                    iter = find(vectorFile.PASSNUMBER == PASS, 1, 'last');
                    
                    % Number of iterations per pass
                    iterationsPerPass(c, PASS) = sum(vectorFile.PASSNUMBER == PASS);
                    
                    % Measure if the pass ended up converging
                    hasConverged(c, PASS) = vectorFile.CONVERGED(iter);
                    
                    % Read the PIV-calculated velocities
                    U = vectorFile.U{iter};
                    V = vectorFile.V{iter};
                    R = vectorFile.R{iter};
                    UVAL = vectorFile.UVAL{iter};
                    VVAL = vectorFile.VVAL{iter};
                    RVAL = vectorFile.RVAL{iter};
%                     spatialPeakRatio = vectorFile.SPATIAL_PEAK_RATIO{PASS};
                    ...
                    bigErrInside{PASS} = cat(1, bigErrInside{PASS}, errInside(:));
                    bigErr{PASS} = cat(1, bigErr{PASS}, absVelocityError(:));
                    
                    
                    
%                     if max(rErrDirInside_pass(:) > 1)
%                         keyboard
%                     end
                    
%                     peakRatioInside = spatialPeakRatio(abs(rTrue{PASS}) >= vorticityThreshold * max(rTrue{PASS}(:)));
...
%         rErrDirInside{PASS} = rErrDirectAve{PASS}(abs(rTrue{PASS}) >= vorticityThreshold * max(rTrue{PASS}(:)));
     ...
     % Save the error file
%      save(errorPath, 'X', 'Y', 'uTrue', 'uTrueInside', 'vTrueInside', 'vTrue', 'rTrue', 'rTrueInside', 'rGradInside', 'rDirInside', 'xInside', 'yInside', 'uErrAve', 'vErrAve', 'velocityErrorMagnitudeAve', 'velocityErrorMagnitudeInside', 'rErrDirectAve', 'rErrorDirInside', 'rErrGradAve', 'rErrorGradInside', 'vorticityThreshold', 'spatialPeakRatioInside');
       save(errorPath, 'X', 'Y', 'uTrue', 'uTrueInside', 'vTrueInside', 'vTrue', 'rTrue', 'rTrueInside', 'rGradInside', 'rDirInside', 'xInside', 'yInside', 'uErrAve', 'vErrAve', 'velocityErrorMagnitudeAve', 'velocityErrorMagnitudeInside', 'rErrDirectAve', 'rErrorDirInside', 'rErrGradAve', 'rErrorGradInside', 'vorticityThreshold', 'nDeforms', 'iterationsPerPass', 'hasConverged', 'bigErr', 'bigErrInside');
