...
%            varMed = median(varColOutside);
% 
%            % Difference of each component from the median value (stencil center)
%            varDiff = abs(testValue - varMed);
% 
%            % Difference of all the other values in the stencil from the median
%            varRes = abs(varColOutside - varMed);
% 
%            % Median of the non-center differences from the median
%            varMedianRes = median(varRes);
% 
%            % Normalized differences 
% %            normDiffs(m, n, v) = abs(varDiff / (varMedianRes + EXPECTED_DIFF))^2;
%             normDiffs(m, n, v) = varDiff / (varMedianRes + EXPECTED_DIFF) > THRESH; 
